...
paquetes_set <- c("readstata13", "dplyr", "tidyverse", "Hmisc",
                  "haven","foreign","factoextra","rpca","FactoMineR",
                  "sp","sf")
#                  "rsjon","highcharter","stringi","viridisLite","scales")

#install.packages(paquetes_set)
lapply(paquetes_set, library, character.only=TRUE)

ruta_1 <- "C:\\Users\\et396\\Dropbox\\Docencia\\Impacto\\ML\\L2\\Data"
setwd(ruta_1)
...
# library(sf)
tx <- st_read("C:/Users/et396/Dropbox/Mapas/Departamental/Departamentos.shp", stringsAsFactors=FALSE)
tx %>% names()
tx %>% str()
